...
export default function ProjectsPage() {
  ...
  return (
    <div className="mx-auto max-w-[1000px] w-full h-auto py-6 flex flex-col items-center min-h-screen">
      ...
      <main className="max-w-[800px] w-full space-y-[22px] mt-[38px] mb-[70px] flex-grow">
        ...
        <div className="px-[20px] flex justify-between flex-wrap gap-[20px]">
          {projects.map((project) => (
            <div className="bg-white px-4 py-6 max-w-[370px] w-full rounded-lg flex flex-col justify-between space-y-[14px]" key={project.slug}>
              <Image src={project.image} width={400} height={0} className="h-auto "/>
              <div className="space-y-[12px]">
                ...
                <div className="space-x-2">
                  {project.tags.map((tag) => (
                    <span key={tag} className="text-[#292D32] text-[14px] py-[3px] px-[6px] border border-solid border-[#292D32]">{tag}</span>
                  ))}
                </div>
                <Link legacyBehavior href={project.link} ><a className="block underline">Learn more</a></Link>
              </div>
            </div>
          ))}
        </div>
